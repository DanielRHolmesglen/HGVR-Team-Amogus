    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                v.vertex.xyz += (((half3(0.1,0.1,0)*sin(((mul(unity_ObjectToWorld, v.vertex).r*node_9539)+node_1465)))+(half3(0,0.1,0.1)*cos(((node_9539*mul(unity_ObjectToWorld, v.vertex).g)+node_1465))))*node_7096);
...
/////// Diffuse:
                ...
                float node_7749 = (node_2418*0.6);
                float3 w = float3(node_7749,node_7749,node_7749)*0.5; // Light wrapping
                float3 NdotLWrap = NdotL * ( 1.0 - w );
                float3 forwardLight = max(float3(0.0,0.0,0.0), NdotLWrap + w );
                float3 backLight = max(float3(0.0,0.0,0.0), -NdotLWrap + w ) * float3(node_7749,node_7749,node_7749);
...
////// Emissive:
                float node_3309 = ((0.05*(1.0 - sin((3.141592654*pow(1.0-max(0,dot(normalDirection, viewDirection)),1.0)))))*node_2418);
                float3 emissive = float3(node_3309,node_3309,node_3309);
/// Final Color:
